...
**CRITICAL: Do NOT approximate OKLCH values.** Use the proper sRGB → XYZ → OKLab → OKLCH conversion pipeline. Approximated values will produce visibly wrong colors.

**Recommended: Run this Node.js snippet** for exact conversion of any hex color:
...
// Replace with your actual hex colors:
['#3b82f6','#172642','#fff8f3'].forEach(hexToOklch);
"
```

Use the output directly in themes.ts — never estimate or round aggressively.

**Alternative tools** (for verification only, not as primary method):

1. **Browser DevTools**: Chrome/Firefox color picker can convert between formats.
2. **Online**: [oklch.com](https://oklch.com) provides interactive conversion.
3. **npm**: The `culori` package provides programmatic conversion.
